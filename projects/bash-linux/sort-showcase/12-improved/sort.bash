...
# This is an improved version of solution 12. It revealed to me when I was reading the GNU sort's manual (https://www.gnu.org/software/coreutils/manual/html_node/sort-invocation.html). I discovered I used the idiom the manual mentions, but with an additional, bug-prone swapping (of which an additional hindrance, beside being bug-prone, might be the time cost, but I'm not sure, as on my machine it runs quite fast for 100k data). So, compared to the not improved version, this one avoids the swapping, and additionally replaces the second longer GAWK script with shorter cut.
